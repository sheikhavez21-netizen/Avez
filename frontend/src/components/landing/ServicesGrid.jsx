    ...
    <div className="max-w-7xl mx-auto px-5 sm:px-8">
      ...
      <div className="mt-14 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {SERVICES.map((s, i) => (
          <a
            key={s.name}
            href={waLink(`Hi Relay! I want to book ${s.pkg}`)}
            target="_blank"
            rel="noopener noreferrer"
            data-testid={`service-card-${i + 1}`}
            style={{ transitionDelay: `${i * 90}ms` }}
            className="reveal group relative rounded-3xl overflow-hidden border border-zinc-200 bg-white shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 text-left block"
          >
